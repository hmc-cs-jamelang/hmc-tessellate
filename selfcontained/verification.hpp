...
#define V_SCOPE_EXIT(...) V_SCOPE_EXIT_IMPL(VERIFICATION_ANONYMOUS_VARIABLE, __VA_ARGS__)

#define V_SCOPE_EXIT_IMPL(var, ...) \
auto var {verification::makeScopeExit([&]{__VA_ARGS__})}; \
(void) var;
...
#if defined(VERIFICATION_ON) && !defined(NDEBUG)
